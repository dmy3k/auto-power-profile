...
export class UpowerDbus {
  ...
  connectSignal(signal, callback) {
    ...
    if (this._linePowerProxy) {
      const lpId = this._linePowerProxy.connect(signal, callback);
      if (lpId !== null && lpId !== undefined) {
        this._linePowerConnectionIds.push(lpId);
      }
    }
  }
  ...
  getPowerState() {
    ...
    const batteryState = this._proxy.State;
    const batteryDischarging =
      batteryState === UPower.DeviceState.PENDING_DISCHARGE ||
      batteryState === UPower.DeviceState.DISCHARGING;
    ...
    const energyRate =
      this._proxy.EnergyRate ??
      this._proxy.get_cached_property?.("EnergyRate")?.unpack() ??
      0;

    // Consider weak adapter if discharge rate is above 1W while having AC online
    const weakAdapterSuspected =
      acOnline === true && batteryDischarging && Math.abs(energyRate) > 1.0;

    return {
      onBattery,
      onAC: !onBattery,
      lowBattery,
      weakAdapterSuspected
    };
  }
  ...
  destroy() {
    ...
    if (this._linePowerProxy) {
      for (const id of this._linePowerConnectionIds) {
        this._linePowerProxy.disconnect(id);
      }
      this._linePowerConnectionIds = [];
      this._linePowerProxy = null;
    }
